...
def split_text_into_chunks(text, min_len, max_len, min_last_word):
    ...
    """
    # Normalize text and split by periods, which are natural breaking points.
    ...
    for i, sentence in enumerate(sentences):
        ...
        # Re-add the period, except for the very last sentence if it didn't have one.
        ...
        if current_chunk and len(current_chunk) + len(sentence_with_punct) > max_len:
            # If the current chunk is too short, force the sentence to join it.
            if len(current_chunk) < min_len:
                current_chunk += sentence_with_punct
            else:
                # Check for abbreviations before splitting to avoid orphaned initials.
